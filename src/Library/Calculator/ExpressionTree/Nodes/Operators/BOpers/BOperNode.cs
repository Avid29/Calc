...
public abstract class BOperNode : OperNode
{
    ...
    public override void ReplaceChild(ExpNode node, int index)
    {
        switch (index)
        {
            ...
            case 1:
                ...
                break;
            default:
                throw new ArgumentOutOfRangeException(nameof(index), $"{nameof(BOperNode)} only contains two (2) children. Zero (0) and one (1) are the only indicies.");
        }
    }

    /// <inheritdoc/>
    public override ExpNode GetChild(int index) => index switch
    {
        0 => LeftChild,
        1 => RightChild,
        _ => throw new ArgumentOutOfRangeException(nameof(index), $"{nameof(BOperNode)} only contains two (2) children. Zero (0) and one (1) are the only indicies."),
    };
    ...
    /// <inheritdoc/>
    public override bool IsConstant() => LeftChild.IsConstant() && RightChild.IsConstant();

    /// <inheritdoc/>
    public override bool IsConstantBy(VarValueNode variable) => LeftChild.IsConstantBy(variable) && RightChild.IsConstantBy(variable);
}
