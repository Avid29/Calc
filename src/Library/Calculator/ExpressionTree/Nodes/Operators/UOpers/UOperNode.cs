...
public abstract class UOperNode : OperNode
{
    ...
    public static UOperNode MakeUOperNode(char c) => c switch
    {
    ...
    /// <inheritdoc/>
    public override void AddChild(ExpNode node) => AddChild(node);
    ...
    public override void ReplaceChild(ExpNode node, int index)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(index), $"{nameof(UOperNode)} only contains one (1) child. Zero (0) is the only index.");
        }
    }
    ...
    /// <inheritdoc/>
    public override bool IsConstantBy(VarValueNode variable) => Child.IsConstantBy(variable);
}
