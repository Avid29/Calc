...
{
    private readonly ExpNode[] _values;
    ...
    public void AddRowToRow(MatrixRow row, ExpNode coefficient = null)
    {
        ...
        for (int i = 0; i < _values.Length; i++)
        {
            ...
            if (coefficient == null)
            {
                newValue = QuickOpers.Sum(row[i].Clone(), this[i]);
            }
            else
            {
                newValue = QuickOpers.Sum(QuickOpers.Multiply(row[i].Clone(), coefficient.Clone()), this[i]);
            }
