...
public class Integrator : Operation
{
    ...
    public override ExpNode Execute(PowOperNode node)
    {
        ...
        var exponent = QuickOpers.Add(1, node.RightChild);
        var coefficient = QuickOpers.Reciprical(exponent.Clone());
        var @base = QuickOpers.Pow(node.LeftChild, exponent);
    ...
    private static ExpNode SineTable(SineOperNode node) => node.SineFunction switch
    {
